 ...
 */
export async function getBookings(query: BookingQuery = {}): Promise<{ bookings: Booking[], total: number }> {
...
export async function checkBookingConflicts(
    ...
    const otherBookings = excludeId ? bookings.filter((b) => b.id !== excludeId) : bookings

    // Check for conflicts
    for (const existingBooking of otherBookings) {
        // Only check bookings for the same room and date
        if (existingBooking.className === booking.className && existingBooking.date === booking.date) {
...
export async function getRoomAvailability(
    ...
    startDate: string,
    endDate: string
): Promise<{ date: string, availableHours: { start: string, end: string }[] }[]> {
    const bookings = await readData()

    // Filter bookings for the specified room and date range
    const roomBookings = bookings.filter(b =>
        b.className === roomName &&
        b.date >= startDate &&
        b.date <= endDate
    )
...
export async function getBookingStatistics(): Promise<{
    ...
    for (const booking of bookings) {
        // Count by room
        bookingsByRoom[booking.className] = (bookingsByRoom[booking.className] || 0) + 1
...
export async function backupBookings(): Promise<string> {
    ...
    const backupPath = path.join(process.cwd(), 'data', `bookings-backup-${timestamp}.json`)
